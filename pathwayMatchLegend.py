...
total = 39
perfectMatch = 14/total
pathwayMatch = 25/total
noMatch = 0/total
...
h1 = plt.text( perfectMatch/2, 0., '14', ha="center", va="center", color="white", fontsize=16, fontweight="bold")

plt.text( perfectMatch + pathwayMatch/2, 0., '25', ha="center", va="center", color="white", fontsize=18, fontweight="bold")

#plt.text( perfectMatch + pathwayMatch + noMatch/4., 0., '2', ha="center", va="center", color="white", fontsize=18, fontweight="bold")

fig.subplots_adjust(bottom = 0.5, right = 0.8)


#legend_elements = [Patch(facecolor='#499562', edgecolor='#499562',label='Perfect Match'), Patch(facecolor='#FCD238', edgecolor='#FCD238',label='Pathway Match'), Patch(facecolor='#CF3B2A', edgecolor='#CF3B2A',label='Network Outliers') ]

legend_elements = [Patch(facecolor='#499562', edgecolor='#499562',label='Perfect Match'), Patch(facecolor='#FCD238', edgecolor='#FCD238',label='Pathway Match')]
...
ax.legend(handles=legend_elements, loc='lower center', ncol = 3, fontsize =18, bbox_to_anchor =(0.49, -2), facecolor = 'white')
plt.rcParams.update({'font.size': 18})
ax.set_facecolor('white')

plt.show()

os.chdir(r'C:\ResearchWorkingDirectory\gitRepositories\PlottingImages')
       
fig.savefig( 'scale_Phen_v3.png', format='png', dpi=2400)   
...
##
#m = Chem.MolFromSmiles(r'[O-]C(=O)C(=O)C(=O)c1c(cccc1C([O-])=O)C([O-])=O')
#imageSTR = 'acen_2_v3.png'
#
